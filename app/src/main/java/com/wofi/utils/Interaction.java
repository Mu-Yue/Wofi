...
import static com.wofi.application.MyApplication.responceData6;
...
public class Interaction {
     ...
     * 查询当前借车记录
     */
    public static void currentBorrow(final String username)
    {
        Thread t=new Thread(new Runnable() {
            @Override
            public void run() {
                OkHttpClient client=new OkHttpClient();
                Request request=new Request.Builder()
                        .url(Constants.CURRENTBORROW_URL+username)
                        .build();
                try {
                    Response response=client.newCall(request).execute();
                    responceData6=response.body().string();
                    Log.e("借车记录",responceData6);
                    parseJSONWithGSONcurrentborrow(responceData6);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    private static void parseJSONWithGSONcurrentborrow(String jsonData)
    {
        Gson gson=new Gson();
        currentborrow=gson.fromJson(jsonData,new TypeToken<Journey>(){}.getType());
    }
